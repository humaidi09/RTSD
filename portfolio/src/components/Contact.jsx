...
import { AlertCircle, AtSign, Check, Copy, Loader2, MapPin, MessageSquare, Phone, RotateCcw, Send, User } from 'lucide-react'
import SectionHeading from './ui/SectionHeading'
import Reveal from './ui/Reveal'
import { FacebookIcon, GithubIcon, InstagramIcon, LinkedinIcon, TwitterIcon, WhatsappIcon } from './ui/BrandIcons'
...
export default function Contact() {
  ...
  const socialBtn =
    'grid h-11 w-11 place-items-center rounded-xl border border-hair bg-fill text-muted transition-all duration-200 hover:-translate-y-0.5 hover:border-neonCyan/40 hover:text-neonCyan hover:shadow-[0_0_22px_rgba(6,182,212,0.28)]'
        ...
        <Reveal>
          <div className="flex h-full flex-col gap-4">
            ...
            <div className="flex items-center gap-4 rounded-2xl glass p-5">
              <span className="grid h-12 w-12 shrink-0 place-items-center rounded-xl border border-hair bg-fill text-neon-magenta">
                <MapPin className="h-5 w-5" />
              </span>
              <div className="min-w-0 flex-1">
                <p className="font-mono text-xs text-muted">Based in</p>
                <p className="font-medium text-ink">Sylhet, Bangladesh</p>
              </div>
            </div>
          ...
          {sent ? (
            ...
            <form onSubmit={onSubmit} noValidate className="glass-strong rounded-2xl p-6 sm:p-8">
              ...
              <button
                type="submit"
                disabled={sending}
                className="group mt-2 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-neonCyan to-neonPurple px-5 py-3.5 font-semibold text-void transition-transform duration-200 hover:scale-[1.01] disabled:cursor-not-allowed disabled:opacity-70 glow-cyan sm:w-auto"
              >
                {sending ? (
                  <>
  ...
  return (
    ...
    >
      <motion.span
        ...
        className="grid h-16 w-16 place-items-center rounded-full border border-neonCyan/40 bg-neonCyan/10 text-neonCyan shadow-[0_0_30px_-6px_rgba(6,182,212,0.6)]"
      >
