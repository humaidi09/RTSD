...
// Spec-mandated spotlight glow on hover, applied to each skill chip.
const CHIP =
  'group/chip flex items-center gap-3 rounded-xl border border-hair bg-fill px-3.5 py-2.5 transition-all duration-200 hover:-translate-y-0.5 hover:bg-fill-2 hover:border-cyan-500/50 hover:shadow-[0_0_20px_rgba(6,182,212,0.25)]'
              ...
              <ul className="mt-6 space-y-2.5">
                {group.items.map((item) => (
                  <li key={item} className={CHIP}>
                    <span className={`${ACCENT_TEXT[group.accent]} transition-colors group-hover/chip:text-cyan-400`}>
